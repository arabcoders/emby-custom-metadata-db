...
public class Utils
{
    ...
    public static Dictionary<string, string> GetEpisodeNfo(string path)
    {
        ...
        try
        {
            ...
            Logger?.Info($"GetEpisodeNfo() - nfoFile: {nfoPath}");
            ...
            try
            {
                ...
                if (root == null)
                {
                    Logger?.Error($"GetEpisodeNfo() - No <episodedetails> found in: {nfoPath}");
                    return info;
                }
            }
            catch (Exception ex)
            {
                Logger?.Error($"GetEpisodeNfo() - Failed to parse XML: {ex.Message}");
                return info;
            }
        ...
        {
            Logger?.Error($"GetEpisodeNfo() - Exception: {ex.Message}");
